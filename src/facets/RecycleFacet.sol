...
contract RecycleFacet {
    ...
    function addRecycleRecord(
        ...
        s.recyclingHistory[msg.sender].push(
            AppStorage.CampusMaterialRecord({user: msg.sender, record: record})
        );

        (bool ok,) = address(this).delegatecall(
            abi.encodeWithSignature(
                "mintReward(address,(uint256,uint256,uint256,uint256,uint256))",
                msg.sender,
                record
            )
        );
    ...
    function getRecycleHistory(address user)
        external
        view
        returns (AppStorage.CampusMaterialRecord[] memory)
    {
